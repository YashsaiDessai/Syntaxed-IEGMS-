...
async def startup_event():
    start_mqtt()
    # Initialize AI Agent with configured Ollama endpoint
...
def configure_agent(req: OllamaConfigRequest):
    ...
    try:
        ...
        return {
            "status": "error",
            "message": f"Failed to reconfigure agent: {str(e)}",
            "config": AppState.ollama_config
        }
...
def get_agent_config():
    ...
    return {
        "config": AppState.ollama_config,
        "agent_active": AppState.grid_agent is not None
    }


# ── AI Agent Decision Endpoints ──────────────────────────────────────────────
